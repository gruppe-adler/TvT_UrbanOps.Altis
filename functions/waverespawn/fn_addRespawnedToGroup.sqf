...
//check player side
if (_side == "UNKNOWN") exitWith {diag_log format ["fn_addRespawnedToGroup - Player %1's side is unknown.", _player]};
if (_side == "WEST") then {
...
if (_player != _leader) then {
  ...
  diag_log format ["fn_addRespawnedToGroup - Player %1 joined %2's group.", _player, _leader];
};
