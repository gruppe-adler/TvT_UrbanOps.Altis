...
if (player getVariable ["wr_interrupted", false]) exitWith {
    ...
    [_explanation, _respawnIn] call uo_fnc_respawnHint;
...
[_respawning] call uo_fnc_respawnHint;

_waitCondition = if (player getVariable "originalSide" == "WEST") then {{WAVERESPAWNBLU}} else {{WAVERESPAWNOPF}};
[{call (_this select 0)}, {
    [profileName, player getVariable ["originalSide", "UNKNOWN"]] remoteExec ["uo_fnc_removeFromWave",2,false];
    _availableOptions = if (player getVariable ["originalSide", "UNKNOWN"] == "WEST") then {count uo_cv_allCVs} else {1};
    if (_availableOptions > 1) then {[] call uo_fnc_chooseRespawn} else {[] call uo_fnc_doRespawn};
