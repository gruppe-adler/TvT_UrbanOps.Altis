...
mcd_chooseRespawnClick = [
    ...
    "onMapSingleClick",
    {
        _respawnObject = [_pos, 300, player getVariable ["originalSide", "UNKNOWN"]] call mcd_fnc_findNearestRespawnObject;
        ...
        if (isNull _respawnObject || !(_respawnObject getVariable ["uo_respawnObject_isActive", false])) then {
            [objNull] call mcd_fnc_respawnSelectorMarker;
        } else {
            [_respawnObject] call mcd_fnc_respawnSelectorMarker;
        };
    }
