...
//check errors
if (_victim == _shooter) exitWith {diag_log format ["fn_getKillerSide - %1 killed himself.", _victim]};
if (isNull _shooter) exitWith {diag_log format ["fn_getKillerSide - %1's killer was not found.", _victim]};
if (side _shooter == CIVILIAN) exitWith {diag_log format ["fn_getKillerSide - %1 was killed by another civilian: %2", _victim, _shooter]};
