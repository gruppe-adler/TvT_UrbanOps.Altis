    ...
    switch (true) do {
        ...
        case (_killRatio < -5): {
            if (uo_civkillRatio >= -6) then {
                ...
                [WEST] call uo_civs_fnc_setFriendIndep;
            };
            if (50 > random 100) then {[EAST,[0,100]] call uo_civs_fnc_markEnemy};
            if (20 > random 100) then {[selectRandom (missionNamespace getVariable ["uo_civs_allCivs",[]])] call uo_civs_fnc_makeMilitia};
        ...
        case (_killRatio > 5): {
            if (uo_civkillRatio <= 6) then {
                ...
                [EAST] call uo_civs_fnc_setFriendIndep;
            };
