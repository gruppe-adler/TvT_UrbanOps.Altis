...
[{
    ...
    switch (true) do {
        //support blufor
        case (_killRatio < -5): {
            if (uo_civkillRatio >= -6) then {[WEST,'Report','Civilians are starting to actively resist the occupying forces.'] remoteExec ['uo_common_fnc_sideNotification',0,false]};
            if (50 > random 100) then {[EAST,[0,100]] call uo_civs_fnc_markEnemy};
            if (20 > random 100) then {[selectRandom (missionNamespace getVariable ["uo_cv_allCivs",[]])] call uo_civs_fnc_makeMilitia};
            if (6 > random 100) then {[EAST,[0,20],"RESPAWNOBJECT"] call uo_civs_fnc_markEnemy};

        };

        case (_killRatio < -3): {
            if (uo_civkillRatio >= -3) then {[WEST,'Report','More civilians are starting to support your cause.'] remoteExec ['uo_common_fnc_sideNotification',0,false]};
        ...
        //support opfor
        case (_killRatio > 5): {
            if (uo_civkillRatio <= 6) then {[EAST,'Report','Civilians are starting to actively resist the invading forces.'] remoteExec ['uo_common_fnc_sideNotification',0,false]};
        ...
        case (_killRatio > 3): {
            if (uo_civkillRatio <= 3) then {[EAST,'Report','More civilians are starting to support your cause.'] remoteExec ['uo_common_fnc_sideNotification',0,false]};
