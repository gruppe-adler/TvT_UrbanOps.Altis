...
mcd_fnc_endByOpfKilled_preEliminated = {
  [{
    if (({side _x == east} count playableUnits)  + ({side _x == east} count switchableUnits) == 0 && DEALERKILLED && BLUFORINCONTROL) then {
...
mcd_fnc_endByOpfKilled_eliminated = {
  [{
    ...
    if (missionNamespace getVariable ["uo_opfDownSince", 0] > 15) then {
      ...
      missionNamespace setVariable ["uo_opfEliminated", true, true];
      [_this select 1] call CBA_fnc_removePerFrameHandler;
    };
